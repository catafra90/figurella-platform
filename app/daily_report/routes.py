...
def save_daily_report(report):
    ...
    # 1) Write into app/static/reports
    ...
    if not os.path.exists(file_path):
        with pd.ExcelWriter(file_path, engine="openpyxl") as w:
            pd.DataFrame(columns=["Date","client_name","package","revenue"]).to_excel(w, sheet_name="Sales", index=False)
            pd.DataFrame(columns=["Date","name","date","source"]).to_excel(w, sheet_name="Leads", index=False)
            pd.DataFrame(columns=["Date","name","outcome","source"]).to_excel(w, sheet_name="Consultations", index=False)
            pd.DataFrame(columns=["Date","name","provider","description"]).to_excel(w, sheet_name="Opportunities", index=False)
            pd.DataFrame(columns=["Date","attendance_done","no_show"]).to_excel(w, sheet_name="Attendance", index=False)

    # 3) Append rows only if at least one field is filled
    with pd.ExcelWriter(file_path, engine="openpyxl", mode="a", if_sheet_exists="overlay") as w:
        ...
        for key, cols in sections.items():
            ...
            # keep rows where any key column is non-empty
            mask = df[cols].astype(str).apply(lambda r: bool(''.join(r).strip()), axis=1)
            df = df[mask]
            if df.empty or key.capitalize() not in sheets: continue
            start = w.sheets[key.capitalize()].max_row
            df.to_excel(w, sheet_name=key.capitalize(), index=False, header=False, startrow=start)
        ...
        if not att_df.drop(columns="Date").replace('', None).dropna(how='all').empty:
            ...
            att_df.to_excel(w, sheet_name="Attendance", index=False, header=False, startrow=start)
...
def download_report():
    ...
    return send_file(file_path, as_attachment=True, download_name='reports.xlsx')
